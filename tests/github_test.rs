...
#[cfg(feature = "github")]
mod github_extra {
    use arctic_oauth::{Error, GitHub, OAuthProvider};
    ...
    async fn error_as_200() {
        ...
        let err = github
            .validate_authorization_code(&http, "expired-code", None)
            .await
    ...
    fn rejects_code_verifier() {
        ...
        let result = github.authorization_url("state", &["repo"], Some("verifier"));
        assert!(
            result.is_err(),
            "GitHub should reject code_verifier (no PKCE)"
        );
    }

    #[tokio::test]
    async fn refresh_not_supported() {
        use super::common::mock_http_client::MockHttpClient;
        ...
        let err = github
            ...
            .await
            .expect_err("GitHub should not support refresh");

        assert!(
            matches!(err, Error::UnexpectedResponse { status: 501 }),
            "expected 501, got: {err:?}"
        );
    }

    #[test]
    fn revocation_not_supported() {
        let github = GitHub::new("cid", "secret", None);
        assert!(
            !github.supports_token_revocation(),
            "GitHub should not support token revocation"
        );
    }
    ...
    fn authorization_url_with_scopes() {
        ...
        let url = github
            .authorization_url("state123", &["repo", "user:email"], None)
            .unwrap();
