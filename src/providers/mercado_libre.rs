...
/// Configuration for creating a [`MercadoLibre`] client with a custom HTTP client.
///
/// Use this when you need to provide your own [`HttpClient`] implementation
/// (e.g. a pre-configured `reqwest::Client` with custom timeouts or proxies).
/// For the common case, use [`MercadoLibre::new`] which uses the built-in default client.
pub struct MercadoLibreOptions<'a, H: HttpClient> {
...
/// # }
/// ```
pub struct MercadoLibre<'a, H: HttpClient> {
    client_id: String,
...
impl<'a, H: HttpClient> MercadoLibre<'a, H> {
    /// Creates a MercadoLibre client from a [`MercadoLibreOptions`] struct.
    ///
    /// Use this when you need a custom HTTP client. For the common case,
    /// use [`MercadoLibre::new`] instead.
    pub fn from_options(options: MercadoLibreOptions<'a, H>) -> Self {
        Self {
...
impl MercadoLibre<'static, reqwest::Client> {
    /// Creates a new Mercado Libre OAuth 2.0 client using the default HTTP client.
    ///
    /// # Arguments
    ///
    ...
    pub fn new(
        ...
        Self::from_options(MercadoLibreOptions {
...
mod tests {
    ...
    fn make_mercado_libre(http_client: &MockHttpClient) -> MercadoLibre<'_, MockHttpClient> {
        MercadoLibre::from_options(MercadoLibreOptions {
            client_id: "cid".into(),
    ...
    fn new_sets_production_endpoints() {
        ...
        let ml = make_mercado_libre(&mock);
        assert_eq!(ml.authorization_endpoint, AUTHORIZATION_ENDPOINT);
        assert_eq!(ml.token_endpoint, TOKEN_ENDPOINT);
    }

    #[test]
    fn name_returns_mercadolibre() {
        let mock = MockHttpClient::new(vec![]);
        let ml = make_mercado_libre(&mock);
        assert_eq!(ml.name(), "MercadoLibre");
    }
    ...
    fn authorization_url_has_no_scopes_param() {
        let mock = MockHttpClient::new(vec![]);
        let ml = make_mercado_libre(&mock);
        let url = ml.authorization_url("state123", "my-verifier");
    ...
    async fn validate_authorization_code_sends_body_credentials_with_pkce() {
        ...
        assert_eq!(tokens.access_token().unwrap(), "ml-tok");

        let requests = mock.take_requests();
        assert_eq!(requests[0].url, "https://api.mercadolibre.com/oauth/token");
    ...
    async fn refresh_access_token_sends_body_credentials() {
        ...
        let tokens = ml.refresh_access_token("refresh-tok").await.unwrap();
