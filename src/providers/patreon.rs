...
/// # Setup
///
/// 1. Create an OAuth client in the [Patreon Portal](https://www.patreon.com/portal/registration/register-clients).
/// 2. Obtain your client ID and client secret from the client details page.
/// 3. Set the redirect URI in your client settings to match the `redirect_uri` you pass to [`Patreon::new`].
///
/// # Scopes
///
/// Patreon uses space-separated scopes. Common scopes include:
///
/// | Scope | Description |
/// |-------|-------------|
/// | `identity` | Read basic user identity |
/// | `identity[email]` | Read user's email address |
/// | `campaigns` | Read campaign information |
/// | `campaigns.members` | Read campaign members |
///
/// See the full list at <https://docs.patreon.com/#scopes>.
///
/// # Example
///
/// ```rust
...
/// let url = patreon.authorization_url(&state, &["identity", "campaigns"]);
...
impl Patreon<'static, reqwest::Client> {
    ...
    /// # Arguments
    ///
    /// * `client_id` - The OAuth 2.0 client ID from Patreon Portal.
    /// * `client_secret` - The OAuth 2.0 client secret from Patreon Portal.
    /// * `redirect_uri` - The URI Patreon will redirect to after authorization. Must match
    ///   the redirect URI configured in your Patreon client settings.
    ///
    /// # Example
    ///
    /// ```rust
...
impl<'a, H: HttpClient> Patreon<'a, H> {
    ...
    /// Builds the Patreon authorization URL that the user should be redirected to.
    ///
    ...
    /// * `scopes` - The OAuth 2.0 scopes to request (e.g. `&["identity", "campaigns"]`).
    ///
    /// # Example
    ///
    /// ```rust
    /// use arctic_oauth::{Patreon, generate_state};
    ///
    /// let patreon = Patreon::new("client-id", "client-secret", "https://example.com/cb");
    /// let state = generate_state();
    ///
    /// let url = patreon.authorization_url(&state, &["identity"]);
    /// assert!(url.as_str().starts_with("https://www.patreon.com/"));
    /// ```
    ...
    ///
    /// Call this in your redirect URI handler after Patreon redirects back with a `code`
    /// query parameter.
    ///
    /// # Arguments
    ///
    ...
    /// # Errors
    ///
    /// Returns [`Error::OAuthRequest`] if Patreon rejects the code, or
    ...
    ///
    /// Patreon access tokens typically expire after 1 month. If your initial token response
...
mod tests {
    ...
    fn authorization_url_builds_correct_params() {
        ...
        assert!(pairs.contains(&("scope".into(), "identity campaigns".into())));
    ...
    async fn validate_authorization_code_sends_body_credentials() {
        ...
        assert_eq!(tokens.access_token().unwrap(), "patreon-tok");
