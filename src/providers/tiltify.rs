...
/// Configuration for creating a [`Tiltify`] client with a custom HTTP client.
///
/// Use this when you need to provide your own [`HttpClient`] implementation
/// (e.g. a pre-configured `reqwest::Client` with custom timeouts or proxies).
/// For the common case, use [`Tiltify::new`] which uses the built-in default client.
pub struct TiltifyOptions<'a, H: HttpClient> {
...
/// # }
/// ```
pub struct Tiltify<'a, H: HttpClient> {
    client_id: String,
...
impl<'a, H: HttpClient> Tiltify<'a, H> {
    /// Creates a Tiltify client from a [`TiltifyOptions`] struct.
    ///
    /// Use this when you need a custom HTTP client. For the common case,
    /// use [`Tiltify::new`] instead.
    pub fn from_options(options: TiltifyOptions<'a, H>) -> Self {
        Self {
...
impl Tiltify<'static, reqwest::Client> {
    ...
    /// HTTP client, use [`Tiltify::from_options`] instead.
    ///
    /// # Arguments
    ///
    ...
    pub fn new(
        ...
        Self::from_options(TiltifyOptions {
...
mod tests {
    ...
    fn make_tiltify(http_client: &MockHttpClient) -> Tiltify<'_, MockHttpClient> {
        Tiltify::from_options(TiltifyOptions {
            client_id: "cid".into(),
    ...
    fn authorization_url_builds_correct_params() {
        let mock = MockHttpClient::new(vec![]);
        let provider = make_tiltify(&mock);
        let url = provider.authorization_url("state123", &["public", "donate"]);

        let pairs: Vec<(String, String)> = url.query_pairs().into_owned().collect();
        assert!(pairs.contains(&("response_type".into(), "code".into())));
        assert!(pairs.contains(&("client_id".into(), "cid".into())));
        assert!(pairs.contains(&("state".into(), "state123".into())));
        assert!(pairs.contains(&("scope".into(), "public donate".into())));
    ...
    async fn validate_authorization_code_sends_body_credentials() {
        ...
        assert_eq!(tokens.access_token().unwrap(), "tiltify-tok");

        let requests = mock.take_requests();
        assert_eq!(requests.len(), 1);
        assert_eq!(requests[0].url, "https://v5api.tiltify.com/oauth/token");
