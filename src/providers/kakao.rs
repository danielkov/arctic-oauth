...
/// Configuration for creating a [`Kakao`] client with a custom HTTP client.
///
/// Use this when you need to provide your own [`HttpClient`] implementation
/// (e.g. a pre-configured `reqwest::Client` with custom timeouts or proxies).
/// For the common case, use [`Kakao::new`] which uses the built-in default client.
///
/// # Example
///
/// ```rust
/// use arctic_oauth::{Kakao, KakaoOptions, HttpClient};
///
...
///     .unwrap();
///
/// let kakao = Kakao::from_options(KakaoOptions {
...
/// });
/// ```
pub struct KakaoOptions<'a, H: HttpClient> {
...
/// # }
/// ```
pub struct Kakao<'a, H: HttpClient> {
    client_id: String,
...
impl<'a, H: HttpClient> Kakao<'a, H> {
    /// Creates a Kakao client from a [`KakaoOptions`] struct.
    ///
    /// Use this when you need a custom HTTP client. For the common case,
    /// use [`Kakao::new`] instead.
    ///
    /// # Example
    ///
    /// ```rust
    /// use arctic_oauth::{Kakao, KakaoOptions};
    ///
    ...
    /// });
    /// ```
    pub fn from_options(options: KakaoOptions<'a, H>) -> Self {
        Self {
...
impl Kakao<'static, reqwest::Client> {
    /// Creates a new Kakao OAuth 2.0 client using the default HTTP client.
    ///
    /// Uses the built-in `reqwest::Client` for HTTP requests. To provide a custom
    /// HTTP client, use [`Kakao::from_options`] instead.
    ///
    /// # Arguments
    ///
    ...
    pub fn new(
        ...
        Self::from_options(KakaoOptions {
...
mod tests {
    ...
    fn make_kakao(http_client: &MockHttpClient) -> Kakao<'_, MockHttpClient> {
        Kakao::from_options(KakaoOptions {
            client_id: "cid".into(),
    ...
    async fn validate_authorization_code_sends_body_credentials() {
        ...
        assert_eq!(requests[0].url, "https://kauth.kakao.com/oauth/token");
