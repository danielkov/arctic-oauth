...
/// Configuration for creating a [`FortyTwo`] client with a custom HTTP client.
///
/// Use this when you need to provide your own [`HttpClient`] implementation
/// (e.g. a pre-configured `reqwest::Client` with custom timeouts or proxies).
/// For the common case, use [`FortyTwo::new`] which uses the built-in default client.
pub struct FortyTwoOptions<'a, H: HttpClient> {
...
/// # }
/// ```
pub struct FortyTwo<'a, H: HttpClient> {
    client_id: String,
...
impl<'a, H: HttpClient> FortyTwo<'a, H> {
    /// Creates a 42 client from a [`FortyTwoOptions`] struct.
    ///
    /// Use this when you need a custom HTTP client. For the common case,
    /// use [`FortyTwo::new`] instead.
    pub fn from_options(options: FortyTwoOptions<'a, H>) -> Self {
        Self {
...
impl FortyTwo<'static, reqwest::Client> {
    /// Creates a new 42 OAuth 2.0 client using the default HTTP client.
    ///
    ...
    /// HTTP client, use [`FortyTwo::from_options`] instead.
    ///
    /// # Arguments
    ///
    ...
    pub fn new(
        ...
        Self::from_options(FortyTwoOptions {
...
mod tests {
    ...
    fn make_forty_two(http_client: &MockHttpClient) -> FortyTwo<'_, MockHttpClient> {
        FortyTwo::from_options(FortyTwoOptions {
            client_id: "cid".into(),
    ...
    fn new_sets_production_endpoints() {
        ...
        let provider = make_forty_two(&mock);
