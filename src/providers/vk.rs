...
/// Configuration for creating a [`VK`] client with a custom HTTP client.
///
/// Use this when you need to provide your own [`HttpClient`] implementation
/// (e.g. a pre-configured `reqwest::Client` with custom timeouts or proxies).
/// For the common case, use [`VK::new`] which uses the built-in default client.
pub struct VKOptions<'a, H: HttpClient> {
...
impl<'a, H: HttpClient> VK<'a, H> {
    /// Creates a VK client from a [`VKOptions`] struct.
    ///
    /// Use this when you need a custom HTTP client. For the common case,
    /// use [`VK::new`] instead.
    pub fn from_options(options: VKOptions<'a, H>) -> Self {
        Self {
...
impl VK<'static, reqwest::Client> {
    ...
    /// HTTP client, use [`VK::from_options`] instead.
    ///
    /// # Arguments
    ///
...
mod tests {
    ...
    fn make_vk(http_client: &MockHttpClient) -> VK<'_, MockHttpClient> {
        VK::from_options(VKOptions {
            client_id: "cid".into(),
