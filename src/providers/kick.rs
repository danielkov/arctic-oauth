...
///
/// ```rust
/// use arctic_oauth::{Kick, ReqwestClient, generate_state, generate_code_verifier};
///
...
/// kick.revoke_token(&http, tokens.access_token()?).await?;
/// # Ok(())
/// # }
/// ```
pub struct Kick {
...
impl Kick {
    /// Creates a Kick client with custom endpoint URLs.
    ///
...
impl Kick {
    ...
    ///
    /// ```rust
    /// # use arctic_oauth::{Kick, ReqwestClient};
    /// # async fn example() -> Result<(), arctic_oauth::Error> {
    ...
    /// let http = ReqwestClient::new();
    ///
    /// kick.revoke_token(&http, "token-to-revoke").await?;
    /// # Ok(())
    /// # }
    /// ```
...
mod tests {
    ...
    fn new_sets_production_endpoints() {
        let kick = Kick::new("cid", "secret", "https://app/cb");
    ...
    async fn revoke_token_sends_body_credentials() {
        ...
        let result = kick.revoke_token(&mock, "tok-to-revoke").await;
        assert!(result.is_ok());
    ...
    async fn revoke_token_non_200_returns_error() {
        ...
        let result = kick.revoke_token(&mock, "tok").await;
        assert!(matches!(
            result,
